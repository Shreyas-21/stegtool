...
def get_payload_bytes(secret_text):
	...
	return len(secret_text).to_bytes(HEADER_LENGTH, byteorder='big') + bytes(secret_text, 'ascii')
...
def get_permutation(rng, payload_size, image_size):
	...
	for i in range(payload_size):
		try:
			...
			return 0
		permutation[i], permutation[rand_ind] = permutation[rand_ind], permutation[i]
...
def emd_f_value(pixel_group, base):
...
def emd_embed_in_group(pixel_group, digit, base):
	...
	f_value = emd_f_value(pixel_group, base)
...
def emd_encrypt(pixels, payload_bytes, indices):
	...
	message_length = int.from_bytes(payload_bytes[0:HEADER_LENGTH], byteorder='big')
	...
	for i in range(0, n * len(payload_digits), n):
		...
		current_pixel_group = emd_embed_in_group(current_pixel_group, payload_digits[int(i / n)], base)
...
def encrypt(cover_image, secret_text, encrypt_method, password):
	...
	if encrypt_method == 'lsb':
		...
		payload_size = 2 * (header_digits + digits_per_character * (len(payload_bytes) - HEADER_LENGTH))
	...
	if payload_size > image_size:
		...
		pixels = get_pixels(cover_image)
		cover_image.putdata(steg_pixels)
		try:
			cover_image.save("steg_image.png")
		except IOError:
			print("Cannot convert and save!")
			sys.exit(1)
		finally:
			print("Saved as steg_image.png")
...
def emd_decrypt(pixels, image_size, password):
	...
	try:
		...
		header_indices = get_permutation(random, n * k * HEADER_LENGTH, image_size)
		message_size = 0
		#Compute the message length
		for i in range(0, n * k * HEADER_LENGTH, n):
			curr_pixel_group = [pixels[j] for j in header_indices[i:i+n]]
			message_size = message_size * base + emd_f_value(curr_pixel_group, base)
		...
		for i in range(0, len(message_indices), n):
			...
			message_digits.append(emd_f_value(current_pixel_group, base))
...
def main():
	...
	if mode == 'e':
		plaintext_filename = input("Enter the secret text file name: ").rstrip()
		
		try:
			plaintext_file = open(plaintext_filename, mode='r', encoding='ascii', errors='strict')
		except OSError:
		...
		encrypt(img, plaintext, method, password)
